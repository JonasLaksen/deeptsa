...
# print(files[0][files[0].columns[0]])
test = list(map(lambda x: x[x.columns[0:4]],files))
# print(list(zip(files)))
...
for i, row in lol.iterrows():
    line1 = f'{row[4]} & {row[5]} & {row[6]} & 0 & {row["0_x"]} & {row["1_x"]} & {row["2_x"]} & {row["3_x"]} \\\\'
    line2 = f'& & & 1 & {row["0_y"]} & {row["1_y"]} & {row["2_y"]} & {row["3_y"]} \\\\'
    line3 = f'& & & 2 & {row[0]} & {row[1]} & {row[2]} & {row[3]} \\\\'
    line4 = '\midrule'
